...
class Top2Vec:
    ...
    def __init__(self,
        ...
        if not all((isinstance(doc, str) or isinstance(doc, np.str_) or isinstance(doc, int) or isinstance(doc, np.int) or isinstance(doc, float)or isinstance(doc, np.float)) for doc in documents):
            raise ValueError("Documents need to be a list of strings")
        if keep_documents:
